...
float4 main( VS_Output Input ) : SV_Target0
{
	...
	for ( int i = 0; i < 6; i++ )
	{
		float4 shadowCoord = mul( float4( WorldPos.xyz + Normal * m_shadowOffsetBias[ 0 ].x, 1.0f ), m_lightViewProjection[ i ] );
...
#ifdef FILTER
	Composite *= m_lightCubeFilterMap.Sample( m_samLinearB, mul( -LightDir, ( float3x3 )m_cubeFilterRotate ) ).xyz;
#endif
